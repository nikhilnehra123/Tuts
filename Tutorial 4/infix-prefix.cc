...
bool hashigherprec(char x, char y){  // returns true if top element in stack has higher precedence than iterating character
	...
	if(p>q){return true;}
		...
		return(rightassc(x) ?true:false);        // CHANGE TO BE NOTICED AS WHAT THE OPERATOR IN RIGHT WILL BE IN LEFT AFTER TRAVERSAL
	}
}
...
string infix_prefix(string exp1){
	reverse(exp1.begin(),exp1.end());
	for(int i=0;i<exp1.length();++i){
		if(exp1[i]=='('){ exp1[i]=')'; continue;}
		if(exp1[i]==')'){ exp1[i]='(';}
	}
	string exp2=infix_postfix(exp1);
...
int main(){
	...
	string exp1;  
	cin>>exp1;
	cout<<infix_prefix(exp1);

	string exp2;  
	cin>>exp2;
	cout<<infix_prefix(exp2);

	string exp3;  
	cin>>exp3;
	cout<<infix_prefix(exp3);

	string exp4;  
	cin>>exp4;
	cout<<infix_prefix(exp4);
